...
build {
  ...
  # Upload the PowerShell WinGet install script to the VM
  provisioner "file" {
    source      = "./winget-install.ps1"
    destination = "C:\\Windows\\Temp\\winget-install.ps1"
  }

  # Execute the PowerShell script during provisioning
  provisioner "powershell" {
    inline = [
      "Write-Output 'Running Winget Installation Script...'",
      "powershell.exe -ExecutionPolicy Bypass -File C:\\Windows\\Temp\\winget-install.ps1",
  ...
  # Final Sysprep process before capturing the image
  provisioner "powershell" {
    inline = [
